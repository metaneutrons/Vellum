...
export function SimulatorClient() {
  ...
  const drawPixelBuffer = useCallback(
    (buffer: Uint8Array) => {
  ...
  const decryptToken = useCallback(
    async (
      ...
      const serverPubKey = await crypto.subtle.importKey("raw", serverPubRaw, "X25519", false, []);
      ...
      const aesKey = await crypto.subtle.deriveKey(
        {
          ...
          info: new TextEncoder().encode("vellum-token-v1"),
        },
        hkdfKey,
  ...
  const telemetryHeaders = useCallback(
  ...
  const doRender = useCallback(
    async (deviceToken: string, signal: AbortSignal): Promise<number> => {
      setState("rendering");
  ...
  const handleReport = async () => {
    if (!token) {
      appendLog("No token — cannot report");
      return;
    }
    ...
    try {
      const res = await fetch(`${baseUrl}/api/v1/ink/report`, {
        ...
        body: JSON.stringify({
          mac: config.mac,
          issue: "Room issue reported via simulator button",
        }),
      });
  ...
  const handlePowerOff = () => {
    ...
    if (canvas) {
      ...
      if (ctx) {
        ...
        ctx.fillRect(0, 0, dm.width, dm.height);
      }
    }
  };
          ...
          >
            ...
            >
              <button
                ...
                disabled={state === "off"}
                title="Refresh (Button 1)"
                style={{ ...btnStyle, background: state === "off" ? "#333" : "#4ade80" }}
              >
                ↻
              </button>
              <button
              ...
              <button
                onClick={state === "off" ? handlePowerOn : handlePowerOff}
                title={state === "off" ? "Power On" : "Power Off"}
                style={{ ...btnStyle, background: state === "off" ? "#22c55e" : "#ef4444" }}
              >
                ⏻
              </button>
            </div>
            ...
            >
              <div
                title="Power LED (red=charging)"
                style={{
                  width: 8,
                  height: 8,
                  borderRadius: "50%",
                  background: config.powerSource === "usb" ? "#ef4444" : "#333",
                }}
              />
              <div
                title="Status LED (green)"
                style={{
                  width: 8,
                  height: 8,
                  borderRadius: "50%",
                  background: state !== "off" ? "#22c55e" : "#333",
                }}
              />
            </div>
            ...
            {(() => {
              ...
              const cw = isLandscape
                ? Math.max(dm.width, dm.height)
                : Math.min(dm.width, dm.height);
              const ch = isLandscape
                ? Math.min(dm.width, dm.height)
                : Math.max(dm.width, dm.height);
              const maxDim = 500;
        ...
        <div style={{ flex: 1, minWidth: 320, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: "#2d2d3f", borderRadius: 8, padding: 16 }}>
            ...
            >
              ...
              <input
                value={config.mac}
                onChange={(e) => setConfig((c) => ({ ...c, mac: e.target.value }))}
                style={inputStyle}
              />
              ...
              <input
                ...
                onChange={(e) =>
                  setConfig((c) => ({
                    ...
                    batteryVoltage: 3.0 + (+e.target.value / 100) * 1.2,
                  }))
                }
              />
              <label>Power</label>
              <select
                ...
                onChange={(e) =>
                  setConfig((c) => ({ ...c, powerSource: e.target.value as "usb" | "battery" }))
                }
                style={inputStyle}
              >
              ...
              <select
                ...
                onChange={(e) => setConfig((c) => ({ ...c, displayModel: e.target.value }))}
                style={inputStyle}
              >
          ...
          {/* Log */}
          <div
            style={{ background: "#0d0d1a", borderRadius: 8, padding: 12, flex: 1, minHeight: 300 }}
          >
