...
export function ServerUpdateProgress({ progress, elevated = false }: {
  ...
  return (
    <ol aria-label={t("serverUpdateProgress")}
      className={`space-y-2.5 rounded-xl border p-4 ${elevated
      ...
      {updateProgressRows(progress).map(({ phase, state }) => (
        <li key={phase} className="flex items-center gap-3 text-sm">
          <span aria-hidden="true" className={`grid size-5 shrink-0 place-items-center rounded-full ${
            ...
            {state === "done" ? <Check size={14} strokeWidth={2.5} />
              : state === "failed" ? <X size={14} strokeWidth={2.5} />
                : state === "active" ? <LoaderCircle size={15} className="animate-spin" />
                  : <Circle size={11} />}
          </span>
          <span className={state === "pending" ? "text-label-tertiary"
            : state === "failed" ? "text-red font-medium" : "text-label"}>
            {t(PHASE_KEYS[phase])}
          </span>
        </li>
      ))}
