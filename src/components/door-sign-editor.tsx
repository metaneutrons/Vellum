...
{t("default")}
        ...
        {activeDisplay !== "default" && activeDisplay in designOverrides && (
          <button onClick={() => removeOverride(activeDisplay)} className="text-xs text-red hover:underline ml-2 focus-ring rounded">
  {t("removeOverride")}
          </button>
        )}
      </div>
...
{t("free")}
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0" ref={containerRef}>
          ...
          <button onClick={addBox} className="mt-2 inline-flex items-center gap-1 text-sm text-accent hover:underline font-medium focus-ring rounded">
  <Plus size={14} aria-hidden="true" /> {t("addTextBox")}
        ...
        <div className="w-64 shrink-0 space-y-3">
          ...
          <div className="p-3 bg-surface-secondary rounded-lg">
            <label className="block text-xs font-semibold uppercase tracking-wide text-label-tertiary mb-2">{t("background")}</label>
            <input type="file" accept="image/png,image/svg+xml,image/jpeg" onChange={handleBgUpload}
            ...
            {activeDesign.backgroundAssetId && (
              <button onClick={() => updateDesign({ ...activeDesign, backgroundAssetId: null })} className="text-xs text-red hover:underline mt-1 focus-ring rounded">{t("remove")}</button>
            )}
          ...
          {selectedBoxData && (
            <div className="p-3 bg-surface-secondary rounded-lg space-y-2">
              ...
              <textarea value={selectedBoxData.template}
                ...
                className="w-full rounded-md bg-surface border border-separator px-2 py-1 text-sm text-label placeholder:text-label-tertiary h-16 resize-none focus-ring" placeholder="{full_name}" />
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <span className="text-[10px] text-label-tertiary">{Math.round(selectedBoxData.fontSize * 100)}%</span>
                </div>
                <div>
...
{t("bold")}
              ...
              <button onClick={() => deleteBox(selectedBoxData.id)} className="inline-flex items-center gap-1 text-xs text-red hover:underline focus-ring rounded">
