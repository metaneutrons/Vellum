...
export function TextBoxCanvas({ boxes, onChange, width, height, backgroundColor, backgroundUrl, templateVars, label }: Props) {
  ...
  return (
    <div className="space-y-2">
      {label && <span className="text-xs font-semibold uppercase tracking-wide text-label-tertiary">{label}</span>}
      ...
      {selectedBoxData && (
        <div className="p-3 bg-surface-secondary rounded-lg space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => deleteBox(selectedBoxData.id)} className="ml-auto inline-flex items-center gap-1 text-xs text-red hover:underline focus-ring rounded"><Trash2 size={12} aria-hidden="true" /> Delete</button>
          </div>
        </div>
      )}
    </div>
