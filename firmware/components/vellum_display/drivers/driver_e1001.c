/**
 * @file driver_e1001.c
 * @brief reTerminal E1001 display driver — 7.5" B&W/4-Gray (UC8179).
 *
...
static const char *TAG = "drv_e1001";
...
static esp_err_t e1001_init(void) {
    ESP_LOGI(TAG, "Initializing E1001 (UC8179, 800x480 4-Gray)");
    ...
    spi_bus_config_t bus = { .mosi_io_num=EPD_PIN_MOSI, .sclk_io_num=EPD_PIN_SCK, .miso_io_num=-1, .quadwp_io_num=-1, .quadhd_io_num=-1, .max_transfer_sz=EPD_WIDTH*EPD_HEIGHT/4 };
    ...
    eink_reset();
    ESP_LOGI(TAG, "E1001 initialized");
    return ESP_OK;
}

static esp_err_t e1001_draw(const uint8_t *buffer, size_t len) {
    ...
    /* Pack palette indices (0-3) into 2bpp: 4 pixels per byte */
    size_t packed_len = expected / 4;
    uint8_t *packed = malloc(packed_len);
    if (!packed) return ESP_ERR_NO_MEM;

    for (size_t i = 0; i < packed_len; i++) {
        packed[i] = (buffer[i*4] << 6) | (buffer[i*4+1] << 4) | (buffer[i*4+2] << 2) | buffer[i*4+3];
    }
...
static esp_err_t e1001_draw_bitmap(const uint8_t *bitmap, uint16_t bw, uint16_t bh, int16_t ox, int16_t oy) {
...
static esp_err_t e1001_sleep(void) { eink_cmd(0x07); eink_data_byte(0xA5); return ESP_OK; }
static esp_err_t e1001_refresh(void) { eink_cmd(0x20); eink_wait_busy(); return ESP_OK; }

static const display_driver_t s_driver = {
    .model="e1001", .width=EPD_WIDTH, .height=EPD_HEIGHT, .palette_size=4, .quantize="grayscale",
    .init=e1001_init, .draw=e1001_draw, .draw_bitmap=e1001_draw_bitmap, .sleep=e1001_sleep, .refresh=e1001_refresh,
};
