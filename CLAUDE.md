...
- `firmware-pr-build.yml` = compile-only smoke check (one model, e1001/esp32s3)
  on every `firmware/**` PR — catches `-Werror` breaks that otherwise only
  surface at release (the type-limits break that failed v1.2.0's firmware).
...
- **USB-serial provisioning (Improv Wi-Fi Serial over USB-Serial-JTAG) is the
  PRIMARY onboarding path** (shipped firmware-v1.2.1), replacing SoftAP as the
  ...
  **Provision** (`provision-tool.tsx`, Web Serial API). Protocol client:
  ...
  `firmware/components/vellum_serial/` (Improv binary frames interleaved with a
  text console on one UART; console cmds wifi/server/token/info/nvs-erase/reboot).
