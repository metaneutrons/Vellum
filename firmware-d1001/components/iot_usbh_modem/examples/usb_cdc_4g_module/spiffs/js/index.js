...
function networkStatus() {
    ...
    const baseShow = document.getElementById('networkShow')
    baseShow.style.display = 'block'
    const highShow = document.getElementById('customerShow')
...
function deviceStatusUpdate (nowTime, list) {
    ...
    const connectCount = document.getElementById('connect-count')

    const online_time = nowTime / 1000000;
    const h = Math.floor(online_time / 3600 );
    const m = Math.floor((online_time /60 % 60));
    const s = Math.floor((online_time % 60));
...
function createList (list) {
    ...
    for(let i = 0;i < list.length; i++){ //遍历一下json数据
...
function listDataDelete (index) {
    const deleteDeviceDic = per[index]
...
function updateRouterList () {
    ...
    Ajax.get(CONSTANT.GET_STATION_URL, function (res) {
        ...
        for (let i = 0; i < per.length; i++) {
            const stationDic = per[i]
            const online_time = (res.now_time - stationDic.online_time_s)/1000000
            const h = Math.floor(online_time / 3600 )
            const m = Math.floor((online_time /60 % 60))
            const s = Math.floor((online_time % 60))
            const connectTime = h + '小时' + m + '分钟' + s + '秒'
...
function initHash () {
    ...
    dom.addEventListener('click', function(e) {
        ...
        if (e.target.className === 'edit-btn') {
            ...
            const inputDataId = e.target.getAttribute('data-id')
            const editBtn = document.getElementById(inputId)
            const listName = document.getElementById(inputDataId)
