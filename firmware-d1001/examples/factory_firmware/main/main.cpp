...
extern "C" void app_main(void)
{
    ...
    assert(recorder != nullptr && "Failed to create recorder");
    assert((phone->installApp(recorder) >= 0) && "Failed to begin recorder");

    Sensor *sensor = new Sensor();
    assert(sensor != nullptr && "Failed to create sensor");
    assert((phone->installApp(sensor) >= 0) && "Failed to begin sensor");

    LcdTest *lcd_test = new LcdTest();
